...
def download(data):#download novels from NCODE.SYOSETU
    ...
    novel_insert_sql =  """INSERT INTO novels(
                                novelid,
                                title,
                                alternative_title,
                                url,
    ...
    #execute sql and retrieve id
    novelcursor.execute(novel_insert_sql,novel_insert_val)
    #Get list of all identifiers
    identifiers = genres + tags + [author,release,novelstatus,'Unreleased']
    for d in identifiers:
        if(d == author):
            novelcursor.execute("SELECT id FROM identifiers WHERE descriptor = %s",(d,))
            if(novelcursor.rowcount == 0):
                novelcursor.execute("INSERT INTO identifiers (descriptor,type) VALUE (%s,'author')",(d,))
        descriptor_insert_sql = "INSERT INTO novel_identifiers (novelid,identifier_id) SELECT %s,id FROM identifiers WHERE descriptor = %s"
...
def processChapters(chapter_number,section,novel_id,chapter_list,chaptercursor):#Translates chapters and processes them to be easier to read
    sql = "INSERT INTO chapters (novelid,chapter_number,chapter_order,section,chapter_title,content) VALUES (%s,%s,%s,%s,%s,%s)"
